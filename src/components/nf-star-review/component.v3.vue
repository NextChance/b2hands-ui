...
import { computed, ref } from 'vue3'

const inputName = ref('StarReview-' + Math.round(Math.random() * 1000))

const onReviewEmit = 'on-review-emit'
const emit = defineEmits<{
  (e: typeof onReviewEmit, value: number): void;
}>();
...
const formatRating = computed(() => {
  return props.rating.toLocaleString(undefined, { minimumFractionDigits: 1 })
})

const starTypeIdle = (isActive: false) => {
  if (props.isDefaultStar) {
...
const handleReview = (value: number) => {
  if (props.isAnInput) {
    emit(onReviewEmit, value)
  }
}
</script>
